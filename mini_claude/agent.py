...
class Agent:
    ...
    def _build_side_query(self):
        ...
        if self._anthropic_client:
            ...
            async def _sq(system: str, user_message: str) -> str:
                ...
                return "".join(b.text for b in resp.content if b.type == "text") # 返回记忆召回结果，最多有5条
            return _sq
        ...
        for msg in self._anthropic_messages:
            ...
            for block in msg["content"]:
                if isinstance(block, dict) and block.get("type") == "tool_result" and isinstance(block.get("content"), str) and len(block["content"]) > budget:
                    keep = (budget - 80) // 2
                    # 将工具结果截断，保留前keep个字符，后keep个字符，中间用...代替
    ...
    # 将调用的工具结果中含有SNIPPABLE_TOOLS的就截断,使用占位符替换
    def _snip_stale_results_anthropic(self) -> None:
        utilization = self.last_input_token_count / self.effective_window if self.effective_window else 0
        if utilization < SNIP_THRESHOLD: # < 0.6, 则不进行截断
            return
        ...
        for mi, msg in enumerate(self._anthropic_messages):
            ...
            for bi, block in enumerate(msg["content"]):
                if isinstance(block, dict) and block.get("type") == "tool_result" and isinstance(block.get("content"), str) and block["content"] != SNIP_PLACEHOLDER:
                    ...
                    tool_info = self._find_tool_use_by_id(tool_use_id) # 返回工具使用信息，包括工具名称和输入参数
                    if tool_info and tool_info["name"] in SNIPPABLE_TOOLS: 
                        # 如果工具名称在SNIPPABLE_TOOLS中，则将工具结果添加到结果列表中
                        results.append({"mi": mi, "bi": bi, "name": tool_info["name"], "file_path": tool_info.get("input", {}).get("file_path")})

        # 如果结果列表中的工具结果数量小于等于KEEP_RECENT_RESULTS，则不进行截断
        ...
        # 只保留每个文件的最新一次调用，其他都截断
        for indices in seen_files.values():
            if len(indices) > 1:
                for j in indices[:-1]:
                    to_snip.add(j)

        # 会出现调用很多个文件,但只保留最新调用的3个文件,其他的都截断
...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that."""
    ...
    # (1) _build_side_query
    # (2) start_spinner
    # 负责将主agent传给其的message处理之后,返回给主agent
    async def _chat_anthropic(self, user_message: str) -> None:
        # 将用户输入加入_anthropic_messages历史消息中
        self._anthropic_messages.append({"role": "user", "content": user_message})

        # 先根据用户输入和.mini-claude目录下的记忆文件，调用模型进行记忆召回，并返回相关记忆列表
        # Start async memory prefetch (non-blocking, fires once per user turn)
        memory_prefetch: MemoryPrefetch | None = None
        if not self.is_sub_agent:
            sq = self._build_side_query() # 构建侧边查询
            if sq:
        ...
        while True:
            ...
            # 压缩3种情况下的会话历史，保留关键信息，用于会话内存预算检查
            # (1) 压缩调用的tools结果，保留关键信息，用于会话内存预算检查
            # (2) 将调用的工具结果中含有SNIPPABLE_TOOLS的就截断,使用占位符替换
            # (3) 判读会话时间过长时,压缩会话内容
            ...
            if memory_prefetch and memory_prefetch.settled and not memory_prefetch.consumed: # 如果记忆预取对象已 settled 且未 consumed，则消费记忆预取对象
                memory_prefetch.consumed = True
                try:
                    memories = memory_prefetch.task.result()
                    if memories:
                        injection_text = format_memories_for_injection(memories) # 将相关记忆列表格式化成字符串，用于注入到用户消息中
                        last = self._anthropic_messages[-1] if self._anthropic_messages else None # 获取最后一轮用户消息(历史信息)
                        if last and last.get("role") == "user":
                            content = last.get("content", "") # 获取最后一轮用户消息的内容
                            if isinstance(content, str):
                                last["content"] = content + "\n\n" + injection_text # 将相关记忆列表注入到最后一轮用户消息中
                            elif isinstance(content, list):
                                content.append({"type": "text", "text": injection_text}) # 将相关记忆列表注入到最后一轮用户消息中
                        else:
                            self._anthropic_messages.append({"role": "user", "content": injection_text}) # 将相关记忆列表注入到最后一轮用户消息中
                        for m in memories:
                            self._already_surfaced_memories.add(m.path) # 将相关记忆文件路径添加到已召回的记忆集合中
                            self._session_memory_bytes += len(m.content.encode()) # 计算相关记忆内容的大小，用于会话内存预算检查
                except Exception:
